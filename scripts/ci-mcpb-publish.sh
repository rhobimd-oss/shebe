...
#   MCP_PRIVATE_KEY     - Ed25519 private key (64 hex chars) for DNS verification
#
# Required files:
...
verify_publication() {
    ...
    local response
    response=$(curl -s "${MCP_REGISTRY_API}/servers?search=shebe" || echo '{"servers":[]}')

    # Check if our server is in the results (API returns .servers[].server.name)
    local found
    found=$(echo "${response}" | jq -r --arg name "${name}" \
        '.servers[]? | select(.server.name == $name) | .server.name' 2>/dev/null | head -1 || echo "")

    if [[ -n "${found}" ]]; then
        log "Server found in registry: ${found}"
        if [[ -n "${version}" ]]; then
            # Find the latest version (isLatest: true)
            local reg_version
            reg_version=$(echo "${response}" | jq -r --arg name "${name}" --arg ver "${version}" \
                '.servers[]? | select(.server.name == $name and .server.version == $ver) | .server.version' \
                2>/dev/null | head -1 || echo "")
            if [[ -n "${reg_version}" ]]; then
                log "Version ${version} found in registry"
            else
                # Show what versions are available
                local available
                available=$(echo "${response}" | jq -r --arg name "${name}" \
                    '.servers[]? | select(.server.name == $name) | .server.version' 2>/dev/null | tr '\n' ' ')
                log "Version ${version} not yet visible. Available: ${available}"
            fi
        fi
        return 0
    else
...
main() {
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --help|-h)
                ...
                echo "  MCP_PRIVATE_KEY    Ed25519 private key (64 hex chars)"
    ...
    if [[ -z "${MCP_PRIVATE_KEY:-}" ]]; then
        error "MCP_PRIVATE_KEY environment variable not set (expected 64 hex chars)"
    fi
